...
public class MiniWidgetSkyViperLinkVideo : BaseVideoWidget() {

    companion object {
        private val filter = IntentFilter(AttributeEvent.STATE_CONNECTED)

        @JvmStatic protected val TAG = MiniWidgetSkyViperLinkVideo::class.java.simpleName
    }
    ...
    private val textureView by lazy(LazyThreadSafetyMode.NONE) {
        view?.findViewById(R.id.skyviper_video_view) as TextureView?
    }

    private val videoStatus by lazy(LazyThreadSafetyMode.NONE) {
        view?.findViewById(R.id.skyviper_video_status) as TextView?
    }
